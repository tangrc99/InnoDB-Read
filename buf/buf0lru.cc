...
buf_block_t *buf_LRU_get_free_only(buf_pool_t *buf_pool) {
  ...
  while (block != nullptr) {
    ...
    // block 包含一个 page 变量，所以两者取地址是等价的
    UT_LIST_REMOVE(buf_pool->free, &block->page);
    mutex_exit(&buf_pool->free_list_mutex);

    // 判断 buffer pool 是否处于动态伸缩的过程，并采取相关动作
...
loop:
  ...
  if (buf_pool->try_LRU_scan || n_iterations > 0) {
    ...
    list. */
    /// 这里是尝试找到已经 flush 到硬盘的 page，不会主动做 flush
...
void buf_LRU_block_free_non_file_page(buf_block_t *block) {
  ...
  // 如果 zip 有数据，该数据不在 block 指向的 frame 中，需要额外处理
  if (data != nullptr) {
  ...
  // 释放页时，直接将 page id 进行重置即可，因为 page 在被拿出时会置零
  buf_page_prepare_for_free(&block->page);
  ut_ad(block->page.get_space() == nullptr);
#endif /* !UNIV_HOTBACKUP */

  // 判断 pool 是否需要做动态伸缩，将 block 放入 withdraw 链表或者 freelist
  if (buf_get_withdraw_depth(buf_pool) &&
      buf_block_will_withdrawn(buf_pool, block)) {
    /* This should be withdrawn */
