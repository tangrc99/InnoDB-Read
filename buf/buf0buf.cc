    ...
    std::mutex *mutex)    /*!< in,out: Mutex protecting chunk map. */
{
  ...
  // 首先将 frame 定位到可以被 UNIV_PAGE_SIZE 整除的位置
...
static void buf_pool_create(buf_pool_t *buf_pool, ulint buf_pool_size,
  ...
  if (buf_pool_size > 0) {
    ...
    // 这里是直接分配出 n_chunks * chunk_size大小的空间，也就是 chunks 中每个 chunk 是连续的
    ...
    // 因为这里还没有对数据结构初始化，所以认为 size 是 0
    buf_pool->curr_size = 0;
    chunk = buf_pool->chunks;

    do {
      // 初始化 chunk，如果初始化失败则清理现场，所有chunk 中的 frame 都被放进了 freelist
      /// 该函数较为重要！
    ...
    // 这是一个 page/block 编号来查询的哈希表
    // 哈希表中存有不同 hash_id 对应的锁信息
    buf_pool->page_hash =
...
dberr_t buf_pool_init(ulint total_size, ulint n_instances) {
  ulint i;
  const ulint size = total_size / n_instances;  // 计算每个pool 实例需要的大小
...
static void buf_pool_resize() {
  ...
  for (ulint i = 0; i < srv_buf_pool_instances; i++) {
    buf_pool = buf_pool_from_array(i);
    // 当缓冲区缩小时，计算并设置 withdraw 链表
    if (buf_pool->curr_size < buf_pool->old_size) {
      ulint withdraw_target = 0;

      /// 会保证缩小尺寸后，新的缓冲区起始点在老缓冲区内部
      const buf_chunk_t *chunk = buf_pool->chunks + buf_pool->n_chunks_new;
      const buf_chunk_t *echunk = buf_pool->chunks + buf_pool->n_chunks;
...
withdraw_retry:
  ...
  for (ulint i = 0; i < srv_buf_pool_instances; i++) {
    buf_pool = buf_pool_from_array(i);
    if (buf_pool->curr_size < buf_pool->old_size) {
      /// 这里会尝试对现有的 block 进行驱逐；具体的驱逐逻辑如下：
      /// 1. 查找是否有处于 shrink 区域内的 free block
      /// 2. 驱逐已使用的，处于 shrink 区域内的 block
      /// 其中第二步具有时间限制，如超时未完成则修改 retry
  ...
  // 函数到达这里，表示已经完成了shrink区域的标记，即指向shrink区域的 block 都在 withdraw
  // list 里面；注意这时只是完成了标记工作（不允许继续使用相关区域），并没有开始内存的回收。
  ...
  for (ulint i = 0; i < srv_buf_pool_instances; ++i) {
    ...
    {
      /// 开辟一块新内存，将旧的 chunk 拷贝到新地址
      /// 虽然内存是以 chunk 为单位管理的，但是分配是以整块来分配的
      /// 所以 resize 之后，旧地址是无法使用的
    ...
    /// 上面的调整可能会失败，所以这里要重新计算 Pool size
  calc_buf_pool_size:
  ...
  // 因为 hash 算法以pow2为底，所以尺寸变化 2 倍时，需要修改页哈希
